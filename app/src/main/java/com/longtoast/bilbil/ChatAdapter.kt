...
class ChatAdapter(
    ...
    fun submitMessages(newMessages: List<ChatMessage>) {
        // 같은 리스트 인스턴스를 전달받을 때 내용을 지우지 않도록 스냅샷을 사용한다.
        val snapshot = newMessages.toList()
        messages.clear()
    ...
    private fun formatTime(isoTimeString: String?): String {
        return try {
            ...
            val date = parseServerDate(isoTimeString) ?: return ""
    ...
    private fun formatDateHeader(isoTimeString: String?): String? {
        return try {
            ...
            val date: Date = parseServerDate(isoTimeString) ?: return null
    ...
    private fun parseServerDate(isoTimeString: String): Date? {
